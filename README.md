...
TO RUN THIS REPOSITORY, download the full zip file and run the script locally on your machine. The download will include two subdirectories (one to store results, and one that holds the files where you will store your API keys) that are necessary for the Python scripts to run.
